<template>
  <div class="home-search-mix" v-if="flags.load" @scroll="mainScroll">
    <div class="_song" v-if="flags.lazy >= 1">
      ...
      <SearchAudioItem
        v-for="(item, index) in song.songs"
        :key="index"
        :detail="item"
      />
      <div class="_more" @click="navHanlder(1)">
        查看更多{{ keyword }}的歌曲 >
      </div>
    </div>
...
<script>
// import { mixSearch } from "../../api/else";
import SearchAudioItem from "./Child/SearchAudioItem";
...
export default {
  ...
  methods: {
    mainRequset() {
      Promise.all([
        this.mixSearch(this.keyword, 1018, 30),
        this.mixSearch(this.keyword, 1, 18),
      ]).then((res) => {
    ...
    songUrl(id) {
      return this.axios(`song/url?id=${id}`, false);
    },
    //==========综合搜索=================
    mixSearch(key, type = 1, limit = 40, offset = 0) {
      if (type === 1) {
        return new Promise((resolve, reject) => {
          this.axios(
            `search?keywords=${key}&limit=${limit}&type=${type}&offset=${offset}`
          )
            .then((res) => {
              let songs = res.data.result.songs;
              let songCount = res.data.result.songCount;
              let ids = songs.map((item) => item.id);
              this.songUrl(ids.toString()).then((respon) => {
